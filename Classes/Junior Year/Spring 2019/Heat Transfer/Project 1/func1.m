...
function [A , B, cc, rr] = func1(L1,L2, D, T_0, T_1,k1,k2);
    ...
    for j = 1:cc
       ...
       switch NodeType(i,j)
            ...
            case 1  %% Node 1 , Top Left
                A(n,n) = -k1*((dy/dx) + (dx/dy));  %% Node #
                A(n,n+1) = k1*(dy/(2*dx));     %% Right
                A(n,n+cc) = k1*(dx/(2*dy));    %% Down node 2
                B(n) = -( (dy/(2*dx))*T_0 + (dx/(2*dy))*T_1);  %% Known values
                
            case 2  %% Node 2 , Top left middle stuff
                A(n,n) = -k1*( ((2*dx)/dy) + (dy/dx));
                A(n,n+1) = k1*(dy/(2*dx)); %% Right
                A(n,n-1) = k1*(dy/(2*dx)); %% Right
                A(n,n+cc) = k1*(dx/dy);    %% Down node 2
                B(n) = -(dx/dy)*T_1;  %% Known value(s)
                
            case 3  %% Node 3 , Top Right
                A(n,n) = -k2*( (dx/dy) + (dy/dx) );
                A(n,n-1) = k2*(dy/(2*dx));     %% Left
                A(n,n+cc) = k2*(dx/(2*dy));    %% Down node 
                B(n) = -( (dy/(2*dx))*T_0 + (dx/(2*dy))*T_1); %% Known value(s)   
                
            case 4  %% Node 4 , Left Middle
                A(n,n) = -k1*( (dx/dy) + ((2*dy)/dx) );
                A(n,n+1) = k1*(dy/dx);         %% Right
                A(n,n+cc) = k1*(dx/(2*dy));    %% Down node 
                A(n,n-cc) = k1*(dx/(2*dy));    %% Upper node
            ...
            case 6  %% Node 5 , Middle Right
                A(n,n) = -k2*( ((2*dy)/dx) + (dx/dy) );
                A(n,n-1) = k2*(dy/dx);         %% Left
                A(n,n+cc) = k2*(dx/(2*dy));    %% Down node 
                A(n,n-cc) = k2*(dx/(2*dy));    %% Upper node
                B(n) = -(dy/dx)*T_0;               %% Known value(s) 
            ...
            case 10  %% Node 10 , Top Middle right
                A(n,n) = -k2*( ((2*dx)/dy) + (dy/dx));
                A(n,n+1) = k2*(dy/(2*dx)); %% Right
                A(n,n-1) = k2*(dy/(2*dx)); %% Right
                A(n,n+cc) = k2*(dx/dy);    %% Down node 2
                B(n) = -(dx/dy)*T_0;  %% Known value(s)
